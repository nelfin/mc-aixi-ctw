...
int main(int argc, char *argv[]) {
	...
	//Action and observation are 1, 0 so ctw_tree would predict reward 0 if it "understood" the game
	ctw->update(1);
	ctw->update(0);
	std::cout << ctw->prettyPrint();
	std::cout << "Sequence: " << ctw->printHistory() << std::endl; //Actually predicts 1...
